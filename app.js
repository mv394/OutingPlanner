...
async function seedStaticQuestions() {
    ...
    // Only create docs that are truly new (not previously deleted)
...
window.editChip = function (btn) {
    ...
    if (chip.dataset.fsid) {
        // User-added custom option
        setDoc(doc(db, "customOptions", chip.dataset.fsid), { value: v.trim() }, { merge: true });
    } else {
        // Built-in static option — persist rename
      const staticId = chip.closest("[data-static-id]")?.dataset.staticId;
        if (staticId) {
     updateDoc(doc(staticQuestionsCol, staticId), {
        [`renamedOptions.${oldValue}`]: v.trim()
      }).catch(err => {
...
window.editCard = function (btn) {
    ...
    if (card.dataset.fsid) {
        // User-added custom option
  setDoc(doc(db, "customOptions", card.dataset.fsid), { value: v.trim() }, { merge: true });
    } else {
        // Built-in static option — persist rename
  const staticId = card.closest("[data-static-id]")?.dataset.staticId;
        if (staticId) {
      updateDoc(doc(staticQuestionsCol, staticId), {
 [`renamedOptions.${oldValue}`]: v.trim()
  }).catch(err => {
          input.value = oldValue;
        if (strong) strong.textContent = oldValue;
            alert("Failed to save edit: " + err.message);
            });
      }
    }
...
window.deleteOption = function (btn, kind) {
  ...
  if (!confirm(`Remove "${input.value}" from the poll?`)) return;
    ...
    // Capture staticId BEFORE removing from DOM (el.closest won't work after removal)
    const staticCard = el.closest("[data-static-id]");
    const staticId   = staticCard?.dataset.staticId;

    // Optimistic: remove immediately
    el.remove();

    if (fsid && !fsid.startsWith("temp_")) {
        // User-added custom option — delete from customOptions collection
...
if (next) parent.insertBefore(el, next); else parent.appendChild(el);
 ...
 // Built-in static option — persist removal in the static question doc
...
/* ?? Load static questions (one-time, then custom options) ?? */
async function loadStaticQuestions() {
    ...
    try {
// Clear any previously rendered static cards
    ...
    const newVal   = renamed[opt.value] || opt.value;
         const newLabel = renamed[opt.label] || opt.label;
          return { ...opt, value: newVal, label: newLabel };
                }
           return renamed[opt] || opt;
      })
             };
// "notes" (Suggestions & Comments) always goes after dynamic questions
    ...
    } catch (err) { console.error("Failed to load static questions:", err); }
}
...
function validate() {
    ...
    // ?? Static questions currently visible in the DOM ??
    document.querySelectorAll("[data-static-id]").forEach(card => {
     const sid = card.dataset.staticId;
      if (sid === "notes") return;
        // Only look up type from what's rendered — don't use STATIC_QUESTIONS for deleted ones
  ...
  const title    = card.querySelector("h2")?.textContent || sid;
    ...
    // ?? Dynamic questions currently visible in the DOM ??
...
function collectAnswers() {
    ...
    document.querySelectorAll("[data-qid]").forEach(card => {
      const fsid  = card.dataset.qid;
        const title = card.querySelector("h2")?.textContent || fsid;
  const radios    = card.querySelectorAll('input[type="radio"]:checked');
        ...
        const hiddens   = card.querySelectorAll('input[type="hidden"]'); // ranked
...
window.resetPoll = function () {
    ...
    // Restore submit button — find it inside the form directly
...
lbl.className = "chip";
 ...
 <span class="chip-text">${escHtml(opt)}</span>
  ...
  group.appendChild(lbl);
        ...
        const cells   = d.cells   || {};  // cells[row][col] = text

        const wrap = document.createElement("div");
   wrap.style.overflowX = "auto";
        const table = document.createElement("table");
     table.className = "vote-table editable-table";
        table.id = groupId;
    ...
    // Top-left: vote checkbox header
        const thVote = document.createElement("th");
   thVote.className = "th-vote";
     thVote.textContent = "Vote";
    hRow.appendChild(thVote);

        // Row label header
     const thRow = document.createElement("th");
     thRow.className = "th-row-label";
   thRow.textContent = "Option";
        hRow.appendChild(thRow);

        // Column headers — inline editable, deletable
    cols.forEach(col => {
   const th = document.createElement("th");
            th.className = "th-col";
         th.dataset.col = col;
 th.innerHTML = `
          <span class="cell-text" contenteditable="true" data-action="editTableCol"
  ...
  tr.dataset.row = row;
            ...
            // Vote checkbox cell
            const tdVote = tr.insertCell();
     tdVote.className = "td-vote";
      const voteInp = document.createElement("input");
     voteInp.type  = inputType;
            voteInp.name  = `${name}_vote`;
  voteInp.value = row;
    tdVote.appendChild(voteInp);

        // Row label — inline editable, with delete button
      // (We use the first "column" concept as the row label; row label sits before data cells)
         // Actually: each col gets its own cell. Row itself has a label cell first.
      const tdLabel = tr.insertCell();
     tdLabel.className = "td-row-label";
      const rowLabel = (cells[row] && cells[row]["__label__"]) || row;
  tdLabel.innerHTML = `
        <span class="cell-text row-label-text" contenteditable="true" data-action="editTableRowLabel"
     ...
     // Data cells — one per column, inline editable
            cols.forEach(col => {
 ...
 // Empty placeholder for the +Col header column
            tr.insertCell().className = "td-spacer";
        });

        // "Add row" body row
        const trAddRow = tbody.insertRow();
    trAddRow.className = "tr-add-row";
        const tdAddRow = trAddRow.insertCell();
        tdAddRow.colSpan = cols.length + 3;
        tdAddRow.innerHTML = `
...
/* ?? Passcode ?? */
const PASSCODE = "OT2025"; // Change this to your desired 6-char code
...
document.addEventListener("DOMContentLoaded", () => {
    // Always require passcode on fresh load — sessionStorage only used within same tab session
    // If you want to re-lock, just close the tab
    const unlocked = sessionStorage.getItem("pollUnlocked") === "1";
    if (!unlocked) {
        // Passcode gate stays visible (default state)
...
document.addEventListener("click", async function (e) {
    ...
    if (sBtn) {
        ...
        const card = document.querySelector(`[data-static-id="${sid}"]`);
        if (card) card.remove();
        try {
            // Mark as deleted instead of deleting — prevents seed from recreating it
        await updateDoc(doc(staticQuestionsCol, sid), { deleted: true });
        } catch (err) {
    ...
    if (dBtn) {
      ...
      const card = document.querySelector(`[data-qid="${fsid}"]`);
        if (card) card.remove();
        try {
    ...
    // ?? Dynamic add/remove row actions ??
...
if (!confirm(`Remove "${val}"?`)) return;
  ...
  return;
    ...
    // ?? Edit table row label ??
    if (action === "editTableRow") {
   const oldVal = btn.dataset.val;
        const newVal = prompt("Edit row:", oldVal);
     if (!newVal || !newVal.trim() || newVal.trim() === oldVal) return;
  const card = btn.closest("[data-qid]");
        const opts = JSON.parse(card.dataset.options || "[]");
     const updated = opts.map(o => o === oldVal ? newVal.trim() : o);
...
return;
    ...
    // ?? Edit table column label ??
 if (action === "editTableCol") {
        const oldVal = btn.dataset.val;
     const newVal = prompt("Edit column:", oldVal);
  if (!newVal || !newVal.trim() || newVal.trim() === oldVal) return;
     const card = btn.closest("[data-qid]");
   const cols = JSON.parse(card.dataset.cols || "[]");
     const updated = cols.map(c => c === oldVal ? newVal.trim() : c);
...
return;
  ...
  await updateDoc(doc(db, "questions", fsid), { options: arrayUnion(value) });
    ...
    // ?? Add table column ??
    if (action === "addTableCol") {
   const value = inp ? inp.value.trim() : "";
        if (!value) { if (inp) inp.focus(); return; }
       const card = btn.closest("[data-qid]");
   const cols = JSON.parse(card.dataset.cols || "[]");
        if (cols.includes(value)) { alert("Column already exists."); return; }
  await updateDoc(doc(db, "questions", fsid), { cols: arrayUnion(value) });
        if (inp) inp.value = "";
    }

    // ?? Inline add column (+ Col button in header) ??
    if (action === "addTableColInline") {
  const newCol = prompt("New column name:");
        if (!newCol || !newCol.trim()) return;
        const card = btn.closest("[data-qid]");
     const cols = JSON.parse(card.dataset.cols || "[]");
   if (cols.includes(newCol.trim())) { alert("Column already exists."); return; }
        await updateDoc(doc(db, "questions", fsid), { cols: arrayUnion(newCol.trim()) });
   return;
    }
});

/* ?? Inline cell edit — persist on blur ?? */
document.addEventListener("focusout", async function (e) {
    const span = e.target;
    if (!span.classList.contains("cell-text") || !span.isContentEditable) return;
    const action = span.dataset.action;
    const fsid   = span.dataset.qid;
 const newVal = span.textContent.trim();
    if (!fsid || !newVal) return;

    if (action === "editTableCol") {
  const oldVal = span.dataset.val;
        if (newVal === oldVal) return;
const card = span.closest("[data-qid]");
    const cols = JSON.parse(card.dataset.cols || "[]");
     const updated = cols.map(c => c === oldVal ? newVal : c);
     // Also rename key in cells map for all rows
     const rows = JSON.parse(card.dataset.options || "[]");
        const cellUpdates = {};
     rows.forEach(row => {
            cellUpdates[`cells.${row}.${newVal}`] = (JSON.parse(card.dataset.cells || "{}")[row]?.[oldVal]) || "";
       cellUpdates[`cells.${row}.${oldVal}`] = deleteField ? undefined : "";
   });
 ...
 span.dataset.row = newVal;
  ...
  return;
   ...
   await updateDoc(doc(db, "questions", fsid), { [`cells.${row}.${col}`]: newVal });
  return;
    }
});
