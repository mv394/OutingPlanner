...
/* ?? Real-time listener for static question changes (renames, removals, restores) ?? */
function watchStaticQuestions() {
    // Track which client initiated the last write so we don't re-process our own edits
    onSnapshot(staticQuestionsCol, snapshot => {
        snapshot.docChanges().forEach(change => {
      const fsid = change.doc.id;
            const data = change.doc.data();
          if (change.type !== "modified") return;

 const card = document.querySelector(`[data-static-id="${fsid}"]`);
      if (data.deleted) { if (card) card.remove(); return; }
  if (!card) return;

    const renamed = data.renamedOptions || {};
         const removed = new Set(data.removedOptions || []);
...
inp.checked = !inp.checked;
  ...
  c.querySelector("input").checked = false;
   ...
   wrap.classList.add("selected");
    ...
    q.options.forEach(opt => {
   const isObj   = typeof opt === "object";
   const val     = isObj ? opt.display : opt.display;
    const origVal = isObj ? opt.origVal  : opt.origVal;
        const div     = document.createElement("div");
    div.className = "chip";
...
function loadQuestions() {
   ...
   container.appendChild(buildQuestionCard(change.doc.data(), fsid));
    ...
    const el = document.querySelector(`[data-qid="${fsid}"]`);
           if (!el) return;
         const data = change.doc.data();
       el.dataset.options = JSON.stringify(data.options || []);
    el.dataset.cols    = JSON.stringify(data.cols    || []);
    el.dataset.cells   = JSON.stringify(data.cells   || {});
 ...
 if (groupEl) {
  ...
  cols.forEach(col => {
        const td = tr.insertCell(); td.className = "td-cell";
          const cellVal = (cells[row] && cells[row][col]) || "";
td.innerHTML = `<span class="cell-text" contenteditable="true" data-action="editTableCell"
          data-qid="${fsid}" data-row="${escHtml(value)}" data-col="${escHtml(col)}"></span>`;
    });
    tr.insertCell().className = "td-spacer";
      tbody.insertBefore(tr, addRow);
      }
